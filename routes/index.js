...
const localStrategy = require("passport-local");//to loged-in user when he returns
const userModel = require("./users");
const postModel = require("./posts");
const storyModel = require("./story");
const upload = require("./multer");
const utils = require("../utils/utils");
passport.use(new localStrategy(userModel.authenticate()));//to loged-in user when he returns
...
router.get("/upload"/*, upload.single("image")*/, isLoggedIn, async function (req, res) {
  let user = await userModel.findOne({ username: req.session.passport.user });
...
router.post("/upload", isLoggedIn, upload.single("image"), async function (req, res) {
  /** 
   const user = await userModel.findOneAndUpdate(
   {username: req.session.passport.user},
   {username: req.body.username, name: req.bode.name, bio: req.body.bio},
   {new:true})
   */ 
    const user = await userModel.findOne({
      username: req.session.passport.user,
    ...
    user.picture = req.file.filename;
    await user.save();
    res.redirect("/edit"); //redirect("/profile")
  }
);
...
router.post("/login", passport.authenticate("local", { // "passport.authenticate("local"," means TRY login using Given username & password
    successRedirect: "/feed", // if given data is true then go /feed /profile
    failureRedirect: "/login", // if given data is false then go /login
  }),
  function (req, res) {}
);

router.get("/logout", function (req, res) { //given by passport module inbuilt functionality
...
function isLoggedIn(req, res, next) { //for make protected rout
